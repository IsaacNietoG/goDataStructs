...
Debido a las raíces /Javaescas/ de esta obra que tratamos en la [[Introducción][introducción]], me parece también correcto implementar equivalentes a los tradicionales métodos /equals()/ y /toString()/. Para esto, vamos a hacer un pequeño paseo por la documentación y reglas de estilo de Golang para ver cuál es la mejor manera.

*** Intermedio: equals y toString en GoLang
**** Equals
El método equals de una clase en Java nos permite revisar si dos objetos son iguales *a nivel semántico o de lógica de negocios*, es decir, más que darnos una verificación real de igualdad, nos da una verificación de *equivalencia*.

En Java esto es necesario porque si simplemente utilizamos el operador ==, dos objetos que son equivalentes siempre nos van a dar un resultado negativo, porque *el operador = = compara referencias, no los objetos como tal*. En Golang la cosa cambia, porque el operador = = nos permite también comparar objetos.

La [[https://go.dev/ref/spec#Operators][especificación de Go]] nos indica que podemos usar el operador == para comparar estructuras.

#+begin_quote
[...]Two struct values are equal if their corresponding non-blank field values are equal. The fields are compared in source order, and comparison stops as soon as two field values differ[...]
#+end_quote
...
En la [[https://pkg.go.dev/golang.org/x/exp/slices][implementación oficial de Slices de Go]] podemos encontrar dos métodos interesantes para este asunto: Equal y EqualFunc.
...
Es más, le dimos la suficiente flexiblidad a estas funciones que en realidad *no le estamos pidiendo a EqualFunc que las estructuras guarden los mismos tipos*, siempre que nos proporciona cualquiera que sea el método para comparar balones con globos, se puede realizar.
...
Para eliminar un elemento de la lista -obviamente- primero tenemos que encontrarlo. Entonces, primero tenemos que tratar un algoritmo auxiliar e implementarlo, el cual es el que nos permite encontrar la primera coincidencia de un elemento bajo la lista.

Para esto, iremos recorriendo la lista con el algoritmo intuitivo que se nos ocurre para esto considerando la forma en la que hemos formado nuestra estructura como un conjunto de nodos encadenados.

Primero fijamos un nodo /n/. Si su elemento no es el que buscamos, procedemos a fijar su nodo /siguiente/ como /n/. Caso contrario, podemos detenernos.

#+NAME: recorridoDeNodosSLL
#+begin_src go :eval never :main no :package 'discard :noweb no-export
	var elemento V
	var n nodoSimple := &l.head
	do {
	    elementoFijado = n.elemento
