...
\usepackage{amsmath}
\usepackage{tikz}
\usetikzlibrary{calc, shapes.multipart, positioning, arrows.meta, chains}
\author{Isaac Julián Nieto Gallegos}
\date{\today}
\title{Estructuras de datos con Golang moderno}
\hypersetup{
 pdfauthor={Isaac Julián Nieto Gallegos},
 pdftitle={Estructuras de datos con Golang moderno},
 pdfkeywords={},
 pdfsubject={},
...
\label{sec:orgca9a15e}
\subsection{El arte de la ciencia y la ciencia del arte}
\label{sec:orgc2c4c5d}
...
\subsection{Mi viaje ritual}
\label{sec:orgcbe2712}
...
\subsection{Como leer este trabajo?}
\label{sec:org5fb4c47}
...
\subsection{Ok, pero cómo logro esta última forma?}
\label{sec:orga2a4ec8}
Sección pendiente jajaj
\section{Genéricos}
\label{sec:orgbb6dcd6}
Muchas de las estructuras de datos que se van a revisar en este trabajo son \textbf{\textbf{colecciones}}, es decir, agrupaciones de elementos que permiten repeticiones (por lo que no son conjuntos).

Y nos interesa poder realizar estas operaciones de ``guardar'' y ``retirar'' elementos de este conjunto de una manera repetible y que nos garantice que funcionará sin importar el \textbf{\textbf{tipo}} de los elementos que guardaremos en el conjunto. Para dejar clara esta necesidad, coloquémonos en esta situación:
\subsection{El mundo sin genéricos}
\label{sec:org0dc31d7}
...
Lo cual, si la memoria no nos falla (y no ocupamos memoria, retrocede en el texto un poco) es \textbf{\textbf{exactamente}} el mismo código que usaremos para comparar en la función intMax. Se nos antojaría entonces poder usar una misma función para ambos casos y así no escribir doble (Do Not Repeat Yourself)
...
\subsection{Un mundo con genéricos}
\label{sec:org86b856e}
...
\begin{enumerate}
\item Análisis: El compilador encunetra todas las invocaciones a código genérico
\item Generación de código: Por cada tipo concreto que se use (sea int, string, float64), el compilador va a generar una versión especializada del código genérico.
\item Sustitución: El compilador sustituye las llamadas al código genérico por llamadas al código especializado necesario que haya creado.
\end{enumerate}
...
\section{Interludio: Preparando el entorno de trabajo}
\label{sec:org112974f}
...
Inicialmente, vamos a definir el módulo principal en donde vamos a estar agregando nuestros packages. Para esto, crearemos el archivo \emph{go.mod} con el siguiente contenido:
...
\section{Iteradores}
\label{sec:org2e33d27}
Muchas de las estructuras de datos que vamos a implementar son \textbf{\textbf{Iterables}}. Que significa que son iterables?.
...
Vamos a definir las especificaciones que queremos que cumplan nuestros \textbf{\textbf{Iterables}} mediante una interfaz.
...
Esta, al igual que el resto de las interfaces que vamos a utilizar durante este trabajo, van a vivir bajo el directorio \emph{/interfaces} de nuestro módulo. Y, de una vez vamos a declarar el tipo.
...
\end{verbatim}
\subsection{Operador for-each}
\label{sec:orgf92350a}
...
Es aquí donde usar Go nos va a cambiar un poco el paradigma respecto a esto. Porque para hacer que nuestras estructuras sean compatibles con este operador, tenemos que implementarlas de una forma distinta a como lo habríamos hecho en otros lenguajes. Por eso, el boceto de interfaces que hasta el momento habíamos hecho, no será el final (\emph{jajaj, te cuentié mijx}). Es más, si te vas al código fuente, ni siquiera aparece.

Antes de Go 1.23, esta funcionalidad se habría tenido que implementar de alguna manera relacionada probablemente a \emph{canales}, pero tenemos la suerte y buena fortuna de que justamente en esta versión se implementó finalmente la biblioteca \emph{iter}. Esta biblioteca nos proporciona la capacidad de que nuestros iteradores funcionen con el bucle nativo \emph{for range} que vimos anteriormente.
\subsection{Package iter}
\label{sec:orgc3807eb}
...
\end{verbatim}
\section{Sobre la comparación de elementos en Golang}
\label{sec:org6797d09}

Muchos algoritmos esenciales de las estructuras de datos que vamos a ver en este texto requieren comparar elementos. Más específicamente, \emph{asumen} que existe una forma de saber si dos elementos son \emph{iguales o equivalentes}.
...
\begin{itemize}
\item Los tipos booleanos son comparables. Dos valores booleanos son iguales si ambos son \emph{true} o ambos son \emph{false}
\item Los tipos enteros son comparables. Dos números son iguales si\ldots{} bueno\ldots{} son iguales
\item Los tipos de punto flotante son comparables. Se define su regla de comparación según el estándar \href{https://ieeexplore.ieee.org/document/8766229}{IEEE 754}.
\item Los tipos complejos son comparables. Dos valores complejos son iguales si sus componentes reales e imaginarios son iguales.
\item Las strings son comparables. Dos strings son iguales si coinciden caracter a caracter.
\item Los punteros son comparables. Dos punteros son iguales si apuntan a la misma dirección de memoria.
\item Los canales son comparables. Dos canales son iguales si fueron creados por la misma llamada a \emph{make} o ambos son \texttt{nil}
\item Las interfaces son comparables. Dos interfaces son iguales si sus tipos dinámicos y valores dinámicos son iguales; o si ambas son \texttt{nil}. El concepto de valor y tipo dinámicos se escapa del enfoque de este texto, pero se profundiza en la \href{https://go.dev/ref/spec\#Variables}{especificación oficial de Go}
\end{itemize}
...
Debido a este problema, los algoritmos que \emph{asumen} la existencia de una forma de saber si dos elementos son \emph{iguales} tienen que ser modificados.
...
Dado un algoritmo \emph{f(i)} con \emph{i} la entrada del algoritmo, que necesite de una función \emph{cmp(a, b)} para determinar si \texttt{a==b}, la implementación de \emph{f} derivará dos algoritmos:

Sea \emph{f(i)} el algoritmo original sin cambios, al cual vamos a agregarle la restricción de que retorne un error si detecta que el tipo de la estructura no es comparable.

Sea \emph{fFunc(i, g(a,b))} un algoritmo derivado, el cual en esencia es \emph{f(i)} pero con la diferencia de que, dado que no le damos una restricción como a \emph{f}, el algoritmo no tiene la garantía de que tiene una forma de comparar elementos, el usuario debe de proporcionar la funcion \emph{cmp(a,b)} que usará el algoritmo para funcionar.
...
Es decir, como siempre vamos a definir nuestras estructuras como \texttt{Estructura[V any]}, no tenemos una forma en tiempo de compilación de determinar si el método Equal debe existir o no. Si ya dijimos en la definición de nuestra Estructura que V no tiene requerimentos (implementando \texttt{any}), no podemos luego pedirle que sea \texttt{comparable}. Entonces, esta validación debe de trasladarse a ser realizada en tiempo de ejecución.

Para lograrlo, recurrimos al paquete \emph{reflect}. La solución consiste en realizar una inspección del tipo V justo antes de iniciar el algoritmo. Si la inspección nos dice que el tipo no soporta el operador \texttt{==}, abortamos la operación de forma segura. Esto se traduce a que nuestro método para evaluar si un tipo sería comparable dentro de una función así sería de esta forma:
...
\section{Sobre ToString}
\label{sec:org3c648c8}

El método toString en Java se utiliza para retornar una representación escrita del objeto. Esta representación en su \href{https://docs.oracle.com/javase/8/docs/api/java/lang/Object.html\#toString--}{documentación}
se especifica como ``concisa pero informativa que sea fácil de leer por una persona''. Si nos ponemos a investigar la documentación de Golang en busca de un método similar, podemos encontrar la interfaz \href{https://pkg.go.dev/fmt\#example-Stringer}{fmt.Stringer}. Esta interfaz pide implementar un método String el cual es el equivalente directo de un toString en Java, en el sentido de que es el que le indica a Golang cómo generar dicha representación escrita del objeto.

Para abrazar las raíces \emph{Javaescas} de este texto, todas las estructuras de datos van a implementar esta interface, lo que nos permitirá decir que todas nuestras estructuras de datos tienen una representación escrita.
...
\end{verbatim}
\section{Colecciones}
\label{sec:orgb8027d6}

Casi todas, si no es que directamente todas las estructuras que vamos a implementar son colecciones\ldots{} al intentar escribir esta parte del libro, y al intentar leerla, se antojaría definir lo que es una colección, pero de hecho acabo de tener un Deja Vu a mis clases de Álgebra Superior, donde nos dimos cuenta de que si quisiéramos dar una definición formal para conjunto/colección entonces estamos metidos en un grave problema relacionado con posibles definiciones circulares o vaguedad en las que no lo sean\ldots{} entonces, para evitarnos este problema, vamos a definir nuestras colecciones no desde el significado real de esta palabra, si no desde el comportamiento que habríamos de esperar de una colección. Una colección debería de contener elementos, con los que podríamos realizar las siguientes operaciones:

\begin{itemize}
\item Agregar elementos.
\item Eliminar elementos.
\item Verificar si un elemento existe dentro de la colección.
\item Saber si la colección está vacía.
\item Obtener el número de los elementos de su interior.
\item Limpiar la colección, es decir, despojarla de sus elementos.
\item Determinar si es igual a otra colección.
\item Retornar una representación escrita de sí misma.
\end{itemize}
...
\subsubsection{Nodo Simple}
\label{sec:orgbc37338}
...
\subsubsection{Esqueleteo de la clase}
\label{sec:org73e329d}
...
\begin{itemize}
\item Intenta declarar una variable sin nombre (por eso el \_), lo cual le da la pista al compilador de que no debe de guardarla en memoria

\item La intenta declarar como una instancia de Colección.

\item Le asigna el valor de un puntero hacia una SinglyLinkedList

\item Esta SinglyLinkedList funcionalmente no existe, pues está siendo casteada desde \emph{nil}, en realidad es un puntero fantasma solamente para intentar realizar la acción como tal.
\end{itemize}
...
func (l *SinglyLinkedList[V]) IndiceDeFunc(elemento V, cmp func(V,V) bool) (int, error){...}
\end{verbatim}
\subsubsection{Implementaciones de los métodos}
\label{sec:org6df386c}
...
En cualquiera de los dos casos, las instrucciones en comuń consisten en construir un nuevo nodo \emph{n} e incrementar el contador de elementos en la lista. Posterior a esto, realizamos la verificación necesaria para partir a uno de los dos casos, quedando hasta el momento de esta forma la implementación:
...
Para verificar que la lista es vacía, podemos verificar sencillamente comprobando si el rabo \emph{r} es 'nil'. Si esto se cumple, actualizamos cabeza y rabo a referencias de 'n' y terminamos.
...
Antes de continuar, decir que podemos suponer con seguridad que el puntero \emph{siguiente} de un nodoSimple se inicializa en 'nil', mismo caso para los punteros \emph{head} y \emph{tail} de la estructura en sí. Esto es verdad porque en Golang se cumple que los atributos de tipo puntero ('*T') se inicializan en 'nil', esto no es verdad para todos los atributos, pero concretamente para este caso sí lo es.

Si la lista no es vacía, entonces el rabo \emph{r} es distinto de 'nil'; haremos que el siguiente del rabo sea 'n' y actualizamos la referencia de tail a \emph{n}.
...
Primero que nada, decir que en todos los métodos que tengan versión \emph{normal} y versión \emph{Func} voy a implementar primero la versión Func. Por qué? Porque una vez implementada la versión Func de una de estos métodos, es trivial utilizar el método Func dentro del método original para que funcione correctamente. Lo haremos de la siguiente forma:
...
\end{verbatim}
\item Contiene
\label{sec:org60455df}

Este método es extremadamente parecido a lo que hemos visto previamente. Simplemente necesitamos ir recorriendo la lista nodo por nodo, usar la función comparadora y retornar true cuando encontremos el elemento buscado.
...
Este método no tiene mucha ciencia, principalmente porque no estamos realizando ningún cambio sobre la lista. De manera análoga a lo que vimos previamente, su versión normal queda de la siguiente forma:
